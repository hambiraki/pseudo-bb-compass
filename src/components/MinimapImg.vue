<template>
  <canvas
    id="minimap" v-bind:width="canvasSquareSideLength" v-bind:height="canvasSquareSideLength"
    v-on:mousedown="setActiveWeapon" v-on:mousemove="transform"
    v-on:mouseup="clearActiveWeapon" v-on:mouseout="clearActiveWeapon"
  ></canvas>
</template>

<script setup lang="ts">
import { WeaponOnMinimap, Location } from '@/weapon-on-minimap/weapon-on-minimap';
 ...
 * 画面の幅と高さの小さい方
 */
const canvasSquareSideLength = computed(
  ():number =>{
...
const draw = (): void => {
  const ctx = getMapCanvasContext();
  if(ctx === null){ return; }
  ctx.clearRect(0,0,canvasSquareSideLength.value,canvasSquareSideLength.value);
  try{
    ctx.drawImage(
      minimapImage.value,
      originalXStart, originalYStart,
      originalSquareSideLength, originalSquareSideLength,
      canvasXStart, canvasYStart,
      canvasSquareSideLength.value, canvasSquareSideLength.value
    );
  }catch{};
